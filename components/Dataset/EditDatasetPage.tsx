...
export default function EditDatasetPage({
  ...
  // Fetch dataset details on component mount
  useEffect(() => {
    if (datasetId) {
      fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/DS/${datasetId}`)
        ...
        .then((data: Dataset) => {
          ...
          setSelectedSchema(data.schemaName);
          setSelectedTable(data.tableName);

          // Fetch table columns
          fetch(
            `${process.env.NEXT_PUBLIC_API_BASE_URL}/DS/fields?connectionId=${data.dbConnectionId}&schemaName=${data.schemaName}&tableName=${data.tableName}`
          )
            .then(async (response) => {
              if (!response.ok) {
                const errorText = await response.text();
